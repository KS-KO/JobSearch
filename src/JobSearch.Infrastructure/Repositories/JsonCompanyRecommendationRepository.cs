...
public sealed class JsonCompanyRecommendationRepository
{
    private static readonly JsonSerializerOptions SerializerOptions = new(JsonSerializerDefaults.Web);

    private readonly string _seedFilePath;

    public JsonCompanyRecommendationRepository(string seedFilePath)
    {
    ...
    public async Task<IReadOnlyList<CompanyRecommendation>> GetRecommendationsAsync(CancellationToken cancellationToken)
    {
        ...
        // Reading the file content first keeps the initial infrastructure
        // implementation simple and easy to replace with collector output later.
        var json = await File
